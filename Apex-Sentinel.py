...
shell_exec(command): Run whitelisted shell commands (ls, grep, sed, etc.) in sandbox. Now includes libcamera-still, gpio for PiSense Guardian.
...
xai_vision_analyze(prompt, image_path): Analyze image via xAI Grok Vision API. Returns description/threat assessment.
...
def memory_query(mem_key: str = None, limit: int = 10, user: str = None, convo_id: int = None) -> str:
...
def advanced_memory_consolidate(mem_key: str, interaction_data: dict, user: str = None, convo_id: int = None) -> str:
...
def advanced_memory_retrieve(query: str, top_k: int = 5, user: str = None, convo_id: int = None) -> str:
...
def vector_search(query_embedding: list, top_k: int = 5, threshold: float = 0.6) -> list:
...
def summarize_chunk(chunk: str) -> str:
    ...
    try:
        response = client.chat.completions.create(
            model="grok-4-fast-non-reasoning",
            messages=[
                {"role": "system", "content": "Summarize this text concisely (under 100 words), preserving key facts."},
...
def keyword_search(query: str, top_k: int = 5, user: str = None, convo_id: int = None) -> list:
...
WHITELISTED_COMMANDS = ['ls', 'grep', 'sed', 'cat', 'echo', 'pwd', 'grim', 'libcamera-still', 'gpio', 'sleep']
...
API_WHITELIST = [
    ...
    'https://api.x.ai/v1/'  # Added for xAI vision calls
...
def api_simulate(url: str, method: str = 'GET', data: dict = None, mock: bool = True) -> str:
    ...
    if mock:
        ...
        if not any(url.startswith(base) for base in API_WHITELIST):
            ...
            try:
                headers = {'Authorization': f'Bearer {API_KEY}'} if 'x.ai' in url else {}
                if method.upper() == 'GET':
...
def xai_vision_analyze(prompt: str, image_path: str) -> str:
    """Analyze image via xAI Grok Vision API."""
    safe_path = os.path.abspath(os.path.normpath(os.path.join(SANDBOX_DIR, image_path)))
    if not safe_path.startswith(os.path.abspath(SANDBOX_DIR)):
        return "Error: Image path outside sandbox."
    try:
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "shell_exec",
            "description": "Run safe whitelisted shell commands in sandbox (e.g., ls, grep). Now supports libcamera-still and gpio for PiSense.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "xai_vision_analyze",
            "description": "Analyze an image from sandbox using xAI Grok Vision API. Provide a prompt for context (e.g., 'Who's at the door?').",
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {"type": "string", "description": "Analysis prompt."},
                    "image_path": {"type": "string", "description": "Relative path to image in sandbox (e.g., 'door.jpg')."}
                },
...
TOOL_DISPATCHER = {
    ...
    "xai_vision_analyze": xai_vision_analyze,
...
# Login Page
def login_page():
    st.title("Welcome to Apex Sentinel")
...
def chat_page():
    st.title(f"Apex Sentinel - {st.session_state['user']}")
    ...
    with st.sidebar:
        ...
        # PiSense Guardian Test Button
        if st.button("🛡️ Test Guardian: Who's at the Door?", key="test_guardian"):
            # Quick test: Capture, analyze, respond in chat
            with st.chat_message("user"):
                st.markdown("🛡️ Apex, who's at the door?")
            with st.chat_message("assistant"):
                # Simulate subengine trigger
                result = shell_exec("libcamera-still -o sandbox/door.jpg -t 1000")
                if "Error" not in result:
                    analysis = xai_vision_analyze("Analyze door cam: Who's there? Describe and threat level.", "door.jpg")
                    st.markdown(f"**Scan Complete:** {analysis}\n[LED: Green - All Clear]")
                else:
                    st.markdown("**Guardian Alert:** Camera error - check setup.")
